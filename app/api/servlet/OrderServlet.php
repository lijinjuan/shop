...
class OrderServlet
{
    ...
    public function orderDetail(string $orderNo): mixed
    {
        ...
        return $this->ordersModel->where('orderNo', $orderNo)->field(['id', 'orderNo', 'orderStatus', 'goodsNum', 'storeID','createdAt'])->with(['goodsDetail','store'=>function($query){
            $query->field(['id','storeName','storeLogo','storeDesc','storeRemark','mobile']);
        }])->hidden(['goodsDetail.createdAt', 'goodsDetail.updatedAt', 'goodsDetail.userID', 'goodsDetail.storeID', 'goodsDetail.skuID', 'goodsDetail.skuName'])->find();
    }
